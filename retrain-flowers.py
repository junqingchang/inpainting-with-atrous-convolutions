...
model_dir = 'flowerschkptretrain/'
if not os.path.exists(model_dir):
    os.mkdir(model_dir)
plots_dir = 'flowersplotsretrain/'
...
if __name__ == '__main__':
    ...
    model = torch.load('flowerschkpt/generator-epoch200.pt')
    model.to(device)

    discrim = Discriminator()
    discrim = torch.load('flowerschkpt/discriminator-epoch200.pt')
    discrim.to(device)

    criterion_R = nn.MSELoss()
    criterion_G = nn.BCELoss()

    optimizer_G = optim.SGD(model.parameters(), lr=LEARNING_RATE)
    optimizer_D = optim.SGD(discrim.parameters(), lr=LEARNING_RATE)
